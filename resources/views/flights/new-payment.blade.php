...
<body>
    ...
    for (let i = 0; i < 11; i++) { const year=currentYear + i; yearSelect.append($('<option>', {
        value: year,
        text: year
        }));
        }
        ...
        if (isFormSubmitted) {
            validateCardNumber($(this), cardType);
        } else {
        ...
        // فقط إظهار الخطأ إذا تم تقديم النموذج
        if (isFormSubmitted) {
    ...
    function formatCardNumber(value, cardType) {
        if (cardType==='amex' ) {
            return value.replace(/(\d{1,4})(\d{1,6})?(\d{1,5})?/, function(match, p1, p2, p3) {
                let result=p1; if (p2) result +=' ' + p2;
                if (p3)
                result +=' ' + p3; return result;
            });
        } else {
            return value.replace(/(\d{1,4})/g, function(match,
            p1, offset) {
    ...
    //
    function validateCardNumber($input, cardType) {
            const cardNumber=$input.val().replace(/\s/g, '' );
            const cardPattern=cardPatterns[cardType];
            const isValidLength = cardPattern.length.includes(cardNumber.length);
        if (isValidLength){
        ...
        // التحقق من صحة النموذج عند تقديمه
         $('#paymentForm').on('submit', function(e) {
            ...
            if(!$('#agreeCheckbox').is(':checked')) {
                ...
                if (cardNumber.length === 0 || !validateCardNumber($('#cardNumber'), cardType)) {
                ...
                if (!$('#expiryYear').val()) {
                    $('#expiryYear').addClass('is-invalid'); isValid=false;
                } else {
            ...
            if (cardHolderName.length < 3) {
                $('#cardHolderName').addClass('is-invalid'); isValid=false;
            } else {
            ...
            if (isValid) {
                ...
                // value: $('#cardNumber').val().replace(/\s/g, '')
                value: cardNumber
                }).appendTo(this);
